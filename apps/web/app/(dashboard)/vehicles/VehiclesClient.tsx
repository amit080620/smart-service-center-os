...
import { Car, Plus, Gauge, Search, Hash, Download, Upload } from 'lucide-react';
...
export default function VehiclesClient({
  ...
  const [importResult, setImportResult] = useState<{ created: number; skipped: number; noMatchingCustomer?: number; errors: string[] } | null>(null);
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch('/api/vehicles/import', { method: 'POST', body: formData });
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {importResult && (
          <div className="bg-emerald-950/30 border border-emerald-900/50 text-emerald-200 text-xs rounded-xl p-3">
            Imported {importResult.created} new vehicle{importResult.created === 1 ? '' : 's'}
            {importResult.skipped > 0 && ` · skipped ${importResult.skipped} (duplicate plate or missing data)`}
            {!!importResult.noMatchingCustomer && ` · ${importResult.noMatchingCustomer} skipped (no matching customer phone — import Customers CSV first)`}
